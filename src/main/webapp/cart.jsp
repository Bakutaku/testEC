  ...
  <div class="body">
      ...
      <nav class="navbar navbar-expand-md bg-body-tertiary baColor">
        <div class="container-fluid">
          ...
          <div class="collapse navbar-collapse" id="navbarNav">
            ...
            <!--サイズが大きいときのログインボタン-->
            <div class="col-auto d-none d-md-block m-2">
              <a type="button" href="login.jsp" class="btn btn-outline-light">Login</a>
            </div>
          </div>
        </div>
      </nav>
    ...
    <div class="main">
      ...
      <form action="UpdateCart" method="POST">
        <!--商品-->
        <% NumberFormat format = new DecimalFormat("###,###,###円");%>
        <% Map<String, Cart> list = (Map<String, Cart>) session.getAttribute("cart");%>
        <% int sum = 0; %>
        <% for(String key : list.keySet()){ %>
        	<% Cart cart = list.get(key); %>
        	<% Item item = cart.getItem(); %>
        <div class="cart-item">
          <div class="container">
            <div class="row">
              <div class="col mb-3 mb-sm-0 mt-sm-2">
                <div class="card">
                  <div class="row g-0">
                    <div class="col-md-4 d-flex align-items-center">
                      <img src="<%= item.getImage() %>" class="img-fluid rounded-start">
                    </div>
                    <div class="col-md-8">
                      <div class="card-body">
                      ...
                      <ul class="list-group list-group-flush">
                        ...
                        <li class="list-group-item">
                          <div class="form-floating mb-1">
                            <input type="number" name="item-<%= item.getId() %>" class="form-control item-count" min="1" max="<%=item.getInventory() %>" value="<%= cart.getCount() %>">
                            <label for="floatingInput">個数</label>
                          </div>
                        </li>
                        <li class="list-group-item">
                          <%= format.format(item.getPrice() * cart.getCount()) %> (単価：<%= format.format(item.getPrice()) %>)
                        </li>
                        <li class="list-group-item">
                          <div class="d-grid gap-2">
                            <a class="btn btn-primary" href="info?item=<%=item.getId() %>" target="_blank" rel="noopener" type="button">詳細</a>
                          </div>
                        </li>
                        <li class="list-group-item">
                          <div class="d-grid gap-2 mt-3">
                            <a href="DeleteCart?item=<%=item.getId() %>" class="btn btn-danger" type="button" >取り消す</a>
                          </div>
                        </li>
                      </ul>
                    </div>
        	...
        	<% sum += item.getPrice() * cart.getCount(); %>
        <% } %>
        
        <!--更新ボタン-->
        <div id="updateBtn">
          <div class="d-grid fixed-bottom p-3 update">
      ...
      <form action="check.jsp">
        <div class="next">
          <div class="container mt-4">
            <div class="row">
              <strong class="h3 text-end">合計：<%= format.format(sum) %></strong>
            </div>
          </div>
          <div class="container">
            <div class="row">
              <div class="d-grid mt-1 col">
                <a type="button" href="home" class="btn btn-primary mb-3">Home</a>
              </div>
              <div class="d-grid mt-1 col">
                <button type="submit" class="btn btn-success mb-3">お支払</button>
              </div>
            </div>
          </div>
        </div>
